#!/usr/bin/env python3
"""Belt temperature-tower asset generator (DISCRETE provini design, orca-pr12998).

Unlike the continuous-bar ShidaoSlicer version, the upstream belt temp tower is a
row of DISCRETE provini (one per temperature) replicated along the belt (designed
Y) with a fixed surface gap. Each provino is the user-approved chevron+arc unit
(unit_single.stl, keel-first), with its temperature embossed UPRIGHT on the 50 mm
face. The C++ calib_temp belt branch (Plater.cpp) injects an M104 per zone at
  print_z[i] = i * PITCH * cos(theta)      (theta = belt_slice_rotation_angle = 45)
which lands in the empty gap just before provino i. PITCH below is the shared
geometry contract with that code — keep them in sync.
